<script>
  ...
  function validate_json() {
    ...
    // Change weight unit
    $json_data.volume.weight_unit =
      $json_data.exercises[0].sets[0].weight.weight_unit;
    ...
    for (let i = 0; i < $json_data.exercises.length; i++) {
      for (let j = 0; j < $json_data.exercises[i].sets.length; j++) {
        $json_data.exercises[i].sets[j].reps = Number(
          $json_data.exercises[i].sets[j].reps
        );
...
<div id="content">
  ...
  <div class="separator" id="create">
    ...
    <button
      ...
      id="add-exercise"
      on:click={addexercise}>Submit</button
    >
  </div>
