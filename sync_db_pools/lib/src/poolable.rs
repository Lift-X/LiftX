...
// Build connection pool for rocket + skytable
#[cfg(feature = "sky")]
impl Poolable for skytable::Connection {
    type Manager = ConnectionManager<Connection>;
    type Error = skytable::error::Error;

    fn pool(db_name: &str, rocket: &Rocket<Build>) -> PoolResult<Self> {
        //let config = Config::from(db_name, rocket)?;
        let pool: r2d2::Pool<ConnectionManager<Connection>> = skytable::pool::get("127.0.0.1", 2003, 10).unwrap();
        Ok(pool)
    }
}