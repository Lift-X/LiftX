...
impl<'a> Auth<'a> {
    ...
    pub async fn login(&self, form: &Login) {
        ...
        let user = self.users.get_by_name(&form.name).await?;
        let session = Session {
            id: user.id,
            name: user.name,
            auth_key: key,
    ...
    /// Changes the name of the currently authenticated user
    /// ```
    /// # use rocket_auth::Auth;
    /// # fn func(auth: Auth) {
    /// auth.change_name("JohnDoe".into());
    /// # }
    /// ```
    #[throws(Error)]
    pub async fn change_name(&self, name: String) {
        if self.is_auth() {
            if !validator::validate_email(&name) {
                throw!(Error::InvalidNameAddressError)
            }
            let session = self.get_session()?;
            let mut user = self.users.get_by_id(session.id).await?;
            user.name = name;
